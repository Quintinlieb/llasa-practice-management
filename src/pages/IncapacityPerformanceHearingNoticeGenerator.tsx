...
type HearingNoticeData = PermanentContractFormData & {
  contractReference: string;
  hearingNoticeType: HearingNoticeType;
  effectiveDate: string;
  contractEndDate: string;
...
const IncapacityPerformanceHearingNoticeGenerator = ({
  ...
  const [validatedPreview, setValidatedPreview] = useState<HearingNoticeData | null>(null);
  ...
  const performanceConcernOptions = useMemo(() => {
    const merged = [...PERFORMANCE_CONCERN_TYPES, ...formData.performanceConcernTypes];
    return Array.from(
      new Set(
        merged
          .map((value) => value.trim())
  ...
  const filteredPerformanceConcernTypes = useMemo(() => {
    ...
    if (!query) return performanceConcernOptions;
    return performanceConcernOptions.filter((option) => option.toLowerCase().includes(query));
  }, [customConcernInput, performanceConcernOptions]);
  ...
  const generatePDF = (data: HearingNoticeData, download = false) => {
    const doc = new jsPDF();
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-start gap-2">
                        <Input
                          ...
                          className={`${getNoticeModalInputClass(formData.issueDate.trim().length > 0)} flex-1 cursor-pointer placeholder:!text-[11px] placeholder:!font-normal placeholder:!text-slate-400`}
                        />
                        <input
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-start gap-2">
                        <Input
                          ...
                          className={`${getNoticeModalInputClass(formData.hearingDate.trim().length > 0)} flex-1 cursor-pointer placeholder:!text-[11px] placeholder:!font-normal placeholder:!text-slate-400`}
                        />
                        <input
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div key={hearingTimeFieldVersion} className="relative">
                        ...
                        >
                          <SelectTrigger
                            id="hearingTime"
                            className={`${getNoticeModalSelectTriggerClass(Boolean(formData.hearingTime.trim()) && !hearingTimeFocused)} ${noticeModalDropdownToneClass}`}
                          >
                          ...
                          <SelectContent hideScrollButtons className={noticeModalSelectContentClass}>
                        ...
                        {hearingTimeFocused ? (
                          <div className="absolute inset-0 z-20">
                            <Input
                              ...
                              className={`h-[34px] pr-11 ${getNoticeModalInputClass(Boolean(formData.hearingTime.trim()))}`}
                            />
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        <SelectTrigger
                          id="hearingFormat"
                          className={`${getNoticeModalSelectTriggerClass(Boolean(formData.hearingFormat))} ${noticeModalDropdownToneClass}`}
                        >
                    ...
                    {formData.hearingFormat ? (
                      <div className="space-y-1.5 md:col-span-2">
                        ...
                        <Input
                          ...
                          className={`${getNoticeModalInputClass(Boolean(formData.hearingLocation.trim()))}`}
                        />
                      </div>
                    ) : null}
                    <div className="space-y-1.5 md:col-span-2">
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <button
                                        ...
                                        tabIndex={-1}
                                        className="inline-flex items-center text-slate-400 hover:text-slate-600"
                                        aria-label={`Concern ${index + 1} guidance`}
                                    >
                                      <Info className="h-3.5 w-3.5" />
                                    </button>
                                  </TooltipTrigger>
                            ...
                            <div className="flex items-center gap-1">
                              {Boolean((formData.performanceConcernDescriptions[type] || "").trim()) ? (
                                <Button
                                  type="button"
                                  variant="ghost"
                                  onClick={() => clearConcernDescription(type)}
