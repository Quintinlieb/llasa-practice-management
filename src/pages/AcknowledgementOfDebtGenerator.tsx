...
type AcknowledgementOfDebtFormState = {
  employeeId: string;
  age: string;
...
type AcknowledgementOfDebtData = PermanentContractFormData & {
  certificateReference: string;
  hearingNoticeType: HearingNoticeType;
  effectiveDate: string;
  serviceEndDate: string;
...
const debtReasonOptions = [
  "Salary Advance",
  "Loan Granted",
  "Cash Shortage",
  "Stock Loss",
  "Damage to Property",
  "Lost Property or Equipment",
  "Overpayment",
  "Private Expense Paid on Employee’s Behalf",
  "Unauthorized Use of Funds",
  "Other Agreed Debt",
] as const;
const repaymentFrequencyOptions = ["Weekly", "Fortnightly", "Monthly", "Bimonthly"] as const;
...
type FirstPagePreviewProps = {
  data: AcknowledgementOfDebtData;
  compact?: boolean;
  children?: ReactNode;
...
const AcknowledgementOfDebtGenerator = ({
  embedded = false,
  ...
  const [validatedPreview, setValidatedPreview] = useState<AcknowledgementOfDebtData | null>(null);
  ...
  const steps = ["Employer Details", "Employee Details", "Debt Details"] as const;
  const stepIcons = [Building2, User2, Briefcase] as const;
  const [activeStep, setActiveStep] = useState(0);
  const [employeeSearchOpen, setEmployeeSearchOpen] = useState(false);
  const [employeeSearchQuery, setEmployeeSearchQuery] = useState("");
  const [formData, setFormData] = useState<AcknowledgementOfDebtFormState>({
    employeeId: "",
    age: "",
    companyLogoDataUrl: "",
    logoPlacement: "center",
  ...
  const [isTotalDebtFocused, setIsTotalDebtFocused] = useState(false);
  const [isInstalmentAmountFocused, setIsInstalmentAmountFocused] = useState(false);
  ...
  const sanitizeMoneyInput = (value: string) => {
    ...
    return limitedDecimal !== undefined ? `${limitedWhole}.${limitedDecimal}` : limitedWhole;
  };

  const normalizeMoneyOnBlur = (rawValue: string) => {
  ...
  const handleTotalDebtChange = (value: string) => {
    const nextValue = sanitizeMoneyInput(value);
    setFormData((prev) => ({ ...prev, totalDebt: nextValue }));
  };

  const handleTotalDebtBlur = () => {
    setIsTotalDebtFocused(false);
    const normalized = normalizeMoneyOnBlur(formData.totalDebt);
    setFormData((prev) => ({ ...prev, totalDebt: normalized }));
  };

  const handleInstalmentAmountChange = (value: string) => {
    const nextValue = sanitizeMoneyInput(value);
    setFormData((prev) => ({ ...prev, instalmentAmount: nextValue }));
  };

  const handleInstalmentAmountBlur = () => {
    setIsInstalmentAmountFocused(false);
    const normalized = normalizeMoneyOnBlur(formData.instalmentAmount);
    setFormData((prev) => ({ ...prev, instalmentAmount: normalized }));
  };

  const handleMoneyFieldKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
  ...
  const isEmploymentStepComplete = useMemo(
    () => {
      const hasTotalDebt = Boolean(formData.totalDebt.trim());
      const hasDebtReason = Boolean(formData.debtReason.trim());
      const hasRepaymentStartDate = Boolean(formData.repaymentStartDate);
      const hasInstalmentAmount = Boolean(formData.instalmentAmount.trim());
      const hasRepaymentFrequency = Boolean(formData.repaymentFrequency.trim());
      return Boolean(
        hasTotalDebt &&
          hasDebtReason &&
          hasRepaymentStartDate &&
          hasInstalmentAmount &&
          hasRepaymentFrequency,
      );
    },
    [
      formData.totalDebt,
  ...
  const validateData = () => {
    ...
    checkRequired(formData.totalDebt, "Total debt");
    checkRequired(formData.debtReason, "Debt reason");
    checkRequired(formData.repaymentStartDate, "Repayment start date");
    checkRequired(formData.instalmentAmount, "Instalment amount");
    checkRequired(formData.repaymentFrequency, "Repayment frequency");
    if (missingFields.length) {
      throw new Error(`Please fill in the following required fields: ${missingFields.join(", ")}`);
    }

    const normalizedTotalDebt = Number(formData.totalDebt.replace(/,/g, ""));
    const normalizedInstalmentAmount = Number(formData.instalmentAmount.replace(/,/g, ""));
    if (!Number.isFinite(normalizedTotalDebt) || normalizedTotalDebt <= 0) {
      throw new Error("Please enter a valid total debt amount.");
    }
    if (!Number.isFinite(normalizedInstalmentAmount) || normalizedInstalmentAmount <= 0) {
      throw new Error("Please enter a valid instalment amount.");
    }
    ...
    return {
      ...formData,
      issueDate,
      salaryAmount: normalizedTotalDebt,
      salaryFrequency: "month" as PermanentContractFormData["salaryFrequency"],
      ...
      debtReason: formData.debtReason.trim(),
      repaymentStartDate: formData.repaymentStartDate,
      instalmentAmount: normalizedInstalmentAmount,
      repaymentFrequency: formData.repaymentFrequency.trim(),
    } as AcknowledgementOfDebtData;
  };
  ...
  const getAodBaseClauses = useCallback(
    (data: AcknowledgementOfDebtData): Array<Omit<ClauseDefinition, "id">> => {
      const employeeFullName =
        [data.employeeName, data.employeeSurname].filter(Boolean).join(" ").trim() || "________________________";
      const employerName = formatCompanyDisplayName(profile?.company_name, profile?.company_type) || "________________________";
      const totalDebtText = formatParagraphSalary(data.totalDebt);
      const instalmentText = formatParagraphSalary(data.instalmentAmount);
      const repaymentStartDateText = formatDate(data.repaymentStartDate) || "________________________";
      const repaymentFrequencyText = data.repaymentFrequency || "________________________";
      const debtReasonText = data.debtReason || "________________________";

      return [
        {
          title: "Paragraph 1",
          body: `I, ${employeeFullName}, hereby acknowledge that I am lawfully indebted to ${employerName} in the total amount of ${totalDebtText} arising from ${debtReasonText}.`,
        },
        {
          title: "Paragraph 2",
          body: `I confirm that the above amount is due and payable to the Employer and that I have no valid defence to the indebtedness. I voluntarily agree that the debt shall be repaid in instalments of ${instalmentText} commencing on ${repaymentStartDateText} and thereafter on a ${repaymentFrequencyText} basis until the debt has been paid in full.`,
        },
        {
          title: "Paragraph 3",
          body: "I further expressly and voluntarily consent to the Employer deducting the agreed instalments from my remuneration, provided that any such deduction shall at all times remain within the limits permitted by law.",
        },
        {
          title: "Paragraph 4",
          body: "I shall be entitled to settle the outstanding balance in full at any time before the debt has been fully discharged. Should my employment terminate before the debt has been paid in full, or should I fail to make payment in accordance with this acknowledgement, the full outstanding balance shall immediately become due and payable.",
        },
      ];
    },
  ...
  const generatePDF = (data: AcknowledgementOfDebtData, download = false) => {
    const doc = new jsPDF();
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(18);
    doc.text("Acknowledgement of Debt", pageWidth / 2, y, { align: "center" });
    y += 20;
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);

    const employeeFullName = [data.employeeName, data.employeeSurname].filter(Boolean).join(" ").trim() || "________________________";
    const clauses = mergeClauses(withClauseIds(getAodBaseClauses(data)));
    ...
    clausesWithEdits.forEach((clause) => {
      ...
      paragraphs.forEach((paragraph) => {
        drawWrapped(paragraph, margin, contentWidth, 5.6);
        y += 3;
      });
    });
    y += 8;
    ...
    if (data.issuer?.trim()) {
      doc.setFont("helvetica", "bold");
      doc.text(data.issuer.trim(), margin, y);
      y += 5;
      doc.setFont("helvetica", "normal");
    }
    ...
    const underlinedSegment = `${employeeFullName} (${employeeIdLabel}: ${employeeIdValue})`;
    const receiptText = `I, ${underlinedSegment}, hereby acknowledge that I received this document and that fully understnad the content hereof.`;
    const boxHeight = 34;
    ensureSpace(boxHeight + 6);
    const boxTop = y;
    doc.rect(margin, boxTop, contentWidth, boxHeight);
    y += 5;
    const ackLines = doc.splitTextToSize(receiptText, contentWidth - 4);
    const ackCursorY = y;
    ...
    if (download) {
      ...
      toast({
        ...
        description: "Acknowledgement of Debt has been generated.",
      });
      return;
    }
          ...
          >
            ...
            >
              ...
              {activeStep === 2 && (
                <div className="space-y-3">
                  <div className="grid md:grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <Label htmlFor="totalDebt" className={`${modalFieldLabelClass} inline-flex items-center gap-1`}>
                        Total debt (R) <span className="text-red-500">*</span>
                      </Label>
                      <Input
                        id="totalDebt"
                        type="text"
                        ...
                        value={
                          isTotalDebtFocused
                            ? formatSalaryAmountTypingDisplay(formData.totalDebt)
                            : formatSalaryAmountDisplay(formData.totalDebt)
                        }
                        onFocus={() => setIsTotalDebtFocused(true)}
                        onBlur={handleTotalDebtBlur}
                        onKeyDown={handleMoneyFieldKeyDown}
                        onChange={(e) => handleTotalDebtChange(e.target.value)}
                        placeholder="e.g. 10,000.00"
                        className={getNoticeModalInputClass(Boolean(formData.totalDebt.trim()))}
                      />
                    </div>

                    <div className="space-y-1.5">
                      <Label htmlFor="instalmentAmount" className={`${modalFieldLabelClass} inline-flex items-center gap-1`}>
                        Instalment amount (R) <span className="text-red-500">*</span>
                      </Label>
                      <Input
                        id="instalmentAmount"
                        type="text"
                        inputMode="decimal"
                        value={
                          isInstalmentAmountFocused
                            ? formatSalaryAmountTypingDisplay(formData.instalmentAmount)
                            : formatSalaryAmountDisplay(formData.instalmentAmount)
                        }
                        onFocus={() => setIsInstalmentAmountFocused(true)}
                        onBlur={handleInstalmentAmountBlur}
                        onKeyDown={handleMoneyFieldKeyDown}
                        onChange={(e) => handleInstalmentAmountChange(e.target.value)}
                        placeholder="e.g. 1,500.00"
                        className={getNoticeModalInputClass(Boolean(formData.instalmentAmount.trim()))}
                      />
                    </div>

                    <div className="space-y-1.5">
                      <Label htmlFor="repaymentStartDate" className={`${modalFieldLabelClass} inline-flex items-center gap-1`}>
                      ...
                      <div className="flex items-start gap-2">
                        <Input
                          ...
                          value={formData.repaymentStartDate ? toDisplayDate(formData.repaymentStartDate) : ""}
                          ...
                          className={`${getNoticeModalInputClass(formData.repaymentStartDate.trim().length > 0)} flex-1 cursor-pointer placeholder:!text-[11px] placeholder:!font-normal placeholder:!text-slate-400`}
                        />
                        <input
                          ref={hearingDatePickerRef}
                          type="date"
                          value={formData.repaymentStartDate && /^\d{4}-\d{2}-\d{2}$/.test(formData.repaymentStartDate) ? formData.repaymentStartDate : ""}
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="repaymentFrequency" className={`${modalFieldLabelClass} inline-flex items-center gap-1`}>
                        Repayment frequency <span className="text-red-500">*</span>
                      </Label>
                      <Select
                        value={formData.repaymentFrequency || undefined}
                        onValueChange={(value) => setFormData((prev) => ({ ...prev, repaymentFrequency: value }))}
                      >
                        <SelectTrigger
                          id="repaymentFrequency"
                          className={`${getNoticeModalSelectTriggerClass(Boolean(formData.repaymentFrequency))} ${noticeModalDropdownToneClass}`}
                        >
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="debtReason" className={`${modalFieldLabelClass} inline-flex items-center gap-1`}>
                        Debt reason <span className="text-red-500">*</span>
                      </Label>
                      <Select
                        value={formData.debtReason || undefined}
                        onValueChange={(value) => setFormData((prev) => ({ ...prev, debtReason: value }))}
                      >
                        <SelectTrigger
                          id="debtReason"
                          className={`${getNoticeModalSelectTriggerClass(Boolean(formData.debtReason))} ${noticeModalDropdownToneClass}`}
                        >
                          <SelectValue placeholder="Select debt reason" />
                        </SelectTrigger>
                        <SelectContent className={noticeModalSelectContentClass}>
                          {debtReasonOptions.map((option) => (
                            <SelectItem key={option} value={option} className={noticeModalSelectItemClass}>
                              {option}
                            </SelectItem>
                          ))}
            ...
            {validatedPreview ? (
              <div className="space-y-8">
                <FirstPagePreview data={validatedPreview} profile={profile} logoPreviewUrl={companyLogoPreview || validatedPreview.companyLogoDataUrl}>
                  <div className="w-full space-y-4 pt-2 text-[11px] leading-relaxed text-black">
                    <h2 className="mb-10 text-center text-[18px] font-bold uppercase">Acknowledgement of Debt</h2>
                    {(() => {
                      const clauses = mergeClauses(withClauseIds(getAodBaseClauses(validatedPreview)));
                      const clausesWithEdits = applyClauseEdits(clauses);
                      ...
                      const renderAddClauseControl = (afterId: string | null) => {
                        ...
                        return (
                          <div key={`add-${afterId ?? "start"}`} className="flex justify-center py-1 px-3">
                            <button
                              type="button"
                              onClick={() => openAddClauseForm(afterId)}
                              className="group relative w-full max-w-[calc(100%-1.5rem)] mx-auto py-2 flex justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                            >
                              <span className="relative z-10 inline-flex h-7 w-14 items-center justify-center bg-white text-xs font-medium text-blue-700 transition-all border border-transparent group-hover:font-semibold group-hover:border-blue-600 group-hover:rounded-full">
                      ...
                      return (
                        <>
                            ...
                            return [
                              <div key={clause.id} className="space-y-2 py-1">
                                <div className="flex items-center justify-between gap-2">
                                  <span className="text-[11px] font-semibold text-slate-700">{clause.title}</span>
                                  {isPreviewEditable ? (
                                ...
                                <div className="space-y-3">
                                  ...
                                  {isActiveEditingClauseCustom ? (
                                    <Input
                                      ...
                                      className={getNoticeModalInputClass(customClauseTitleDraft.trim().length > 0)}
                                    />
                                  ) : null}
                                  <Textarea
                                  ...
                                  <div className="flex items-center justify-end gap-2">
                                    {Boolean(clauseEdits[activeEditingClause.id] || customClauseTitleEdits[activeEditingClause.id]) ? (
                                      <Button
                                        size="sm"
                                        variant="ghost"
                          ...
                          {isPreviewEditable && addingAfter !== undefined ? (
                            <div className="fixed inset-x-0 -top-16 bottom-0 z-[999] flex items-center justify-center bg-slate-900/35 px-4">
                              <div className="w-full max-w-3xl rounded border border-slate-200 bg-white p-4 shadow-xl" role="dialog" aria-modal="true" aria-label="Add paragraph">
                                <div className="space-y-3">
                                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="pt-12 pl-0">
                      ...
                      {validatedPreview.issuer?.trim() ? <p className="font-semibold">{validatedPreview.issuer.trim()}</p> : null}
                      <p>Management</p>
                    </div>
                    <div className="mt-6 border border-black p-2 space-y-4">
                      <p>
                        I, <span className="underline">{`${[validatedPreview.employeeName, validatedPreview.employeeSurname].filter(Boolean).join(" ").trim() || "________________________"} (${validatedPreview.idType === "id" ? "ID" : "Passport"}: ${validatedPreview.idType === "id" ? (validatedPreview.employeeIdNumber || "________________________") : (validatedPreview.passportNumber || "________________________")})`}</span>, hereby acknowledge that I received this
                        document and that fully understnad the content hereof.
                      </p>
              ...
              <div className="flex h-full items-center justify-center">
                <p className="text-sm text-muted-foreground">Complete the form to preview the Acknowledgement of Debt.</p>
              </div>
            )}
          </ScrollArea>
                {!useExternalShell ? (
