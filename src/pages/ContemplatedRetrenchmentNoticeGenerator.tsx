  ...
  const searchedEmployees = useMemo(() => {
    ...
    return sortedEmployees
      .map((employee) => {
        ...
        if (idNumberLower === query) score += 1000;
        if (fullNameLower.startsWith(query)) score += 800;
        if (idNumberLower.startsWith(query)) score += 800;
        if (fullNameLower.includes(query)) score += 500;
        if (idNumberLower.includes(query)) score += 500;
        if (firstNameLower.startsWith(query) || surnameLower.startsWith(query)) score += 350;
        if (
          tokens.length > 0 &&
          tokens.every((token) =>
            fullNameLower.includes(token) ||
            firstNameLower.includes(token) ||
            surnameLower.includes(token) ||
            idNumberLower.includes(token),
          )
        ) {
          score += 300;
        }
