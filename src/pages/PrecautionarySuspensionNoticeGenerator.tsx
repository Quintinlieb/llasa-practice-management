...
type PrecautionarySuspensionFormState = {
  ...
  precautionarySuspensionType: PrecautionarySuspensionType | "";
...
type PrecautionarySuspensionType = "general" | "renewal" | "extension";

type PrecautionarySuspensionData = PermanentContractFormData & {
  noticeReference: string;
  precautionarySuspensionType: PrecautionarySuspensionType;
  effectiveDate: string;
  noticeEndDate: string;
...
const precautionarySuspensionTypeOptions: Array<{ value: PrecautionarySuspensionType; label: string }> = [
  { value: "general", label: "Precautionary Suspension Notice" },
  { value: "renewal", label: "Precautionary Suspension Notice (Renewal)" },
  { value: "extension", label: "Precautionary Suspension Notice (Extension)" },
];

const precautionarySuspensionTypeLabels: Record<PrecautionarySuspensionType, string> = {
  general: "Precautionary Suspension Notice",
  renewal: "Precautionary Suspension Notice (Renewal)",
  extension: "Precautionary Suspension Notice (Extension)",
};
...
const HEARING_RIGHTS_INTRO = "Please take note of the following regarding your suspension:";

const HEARING_RIGHTS_ITEMS = [
  "You are hereby placed on paid precautionary suspension pending the above disicplinary hearing.",
  "During this period of suspension it is to be noted that you are not permitted to be on the premises of the company.",
  "You will be expected to hand in all relevant keys, access card and identity card.",
  "You may not remove any documentation or property owned by the company.",
  "You must return to the company any documentation or property in your possession.",
  "During this period you are not permitted to contact the company or any of its employees without prior permission.",
  "Should you wish to contact the company during this period of suspension please use the landline.",
  "Your suspension is on full pay.",
  "You are required to provide the company with your telephone number so that you can be contacted when required. Alternatively, please report to your immediate supervisor every third day during your suspension.",
] as const;
...
const fillClausePlaceholders = (body: string | string[], noticeRef: string, effectiveDate: string, newEndDate = "") => {
  const replaceText = (text: string) =>
    text
      .replace("[notice reference]", noticeRef)
...
type FirstPagePreviewProps = {
  data: PrecautionarySuspensionData;
  compact?: boolean;
  children?: ReactNode;
...
const PrecautionarySuspensionNoticeGenerator = ({
  embedded = false,
  ...
  const [validatedPreview, setValidatedPreview] = useState<PrecautionarySuspensionData | null>(null);
  ...
  const [formData, setFormData] = useState<PrecautionarySuspensionFormState>({
    employeeId: "",
    age: "",
    companyLogoDataUrl: "",
    logoPlacement: "center",
    ...
    misconductTypes: [],
    noticeReference: "",
    precautionarySuspensionType: "general",
    effectiveDate: "",
    noticeEndDate: "",
    newEndDate: "",
    idType: "id",
  ...
  const suspensionStartDatePickerRef = useRef<HTMLInputElement | null>(null);
  const suspensionEndDatePickerRef = useRef<HTMLInputElement | null>(null);
  const hearingDatePickerRef = useRef<HTMLInputElement | null>(null);
  const noticeReferencePickerRef = useRef<HTMLInputElement | null>(null);
  const noticeEndDatePickerRef = useRef<HTMLInputElement | null>(null);
  ...
  const precautionarySuspensionModalDropdownToneClass =
    "bg-white border-slate-300 hover:border-blue-400 data-[state=open]:border-slate-300 data-[state=open]:bg-white";
  const precautionarySuspensionModalSelectContentClass = "!rounded";
  const precautionarySuspensionModalSelectItemClass =
  ...
  const getPrecautionarySuspensionModalInputClass = (isComplete: boolean) =>
    `${baseModalFieldClass} !h-[34px] !border-[1.75px] !border-slate-300 !focus-visible:border-slate-300 ${isComplete ? "!border-emerald-500" : ""}`;
  const getPrecautionarySuspensionModalSelectTriggerClass = (isComplete: boolean) =>
  ...
  const misconductOptions = useMemo(() => {
    const mappedConductOffenceNames = conductOffences.map((offence) => offence.name);
    const merged = [...MISCONDUCT_TYPES, ...mappedConductOffenceNames, ...formData.misconductTypes];
    return Array.from(
      new Set(
        merged
          .map((value) => value.trim())
  ...
  const filteredMisconductTypes = useMemo(() => {
    ...
    return misconductOptions.filter((option) => option.toLowerCase().includes(query));
  ...
  const isEmploymentStepComplete = useMemo(
    () => {
      const hasSuspensionStart = Boolean(formData.suspensionStart);
      const hasSuspensionEnd = Boolean(formData.suspensionEnd);
    ...
    [
      formData.suspensionStart,
      formData.suspensionEnd,
  ...
  useEffect(() => {
    ...
    onStepMetaChange?.({
      ...
      precautionarySuspensionType: formData.precautionarySuspensionType,
    ...
    showFinalActions,
    isPreviewEditable,
    formData.precautionarySuspensionType,
  ]);
  ...
  const resetPrecautionarySuspensionStepFields = () => {
  ...
  const clearCurrentStepFields = () => {
    ...
    if (activeStep === 2) {
      resetPrecautionarySuspensionStepFields();
      return;
    }
    resetForm();
  };
  ...
  const openSuspensionStartDatePicker = () => {
    const picker = suspensionStartDatePickerRef.current;
    if (!picker) return;
    if (typeof (picker as any).showPicker === "function") {
      (picker as any).showPicker();
    } else {
  ...
  const openSuspensionEndDatePicker = () => {
    const picker = suspensionEndDatePickerRef.current;
    if (!picker) return;
    if (typeof (picker as any).showPicker === "function") {
      (picker as any).showPicker();
    } else {
  ...
  const removeMisconductType = (chargeType: string) => {
    ...
    setFormData((prev) => ({
      ...prev,
      misconductTypes: prev.misconductTypes.filter((type) => type !== chargeType),
    }));
  ...
  const openNoticeReferencePicker = () => {
    const picker = noticeReferencePickerRef.current;
    if (!picker) return;
    if (typeof (picker as any).showPicker === "function") {
      (picker as any).showPicker();
    } else {
  ...
  const openNoticeEndDatePicker = () => {
    const picker = noticeEndDatePickerRef.current;
    if (!picker) return;
    if (typeof (picker as any).showPicker === "function") {
      (picker as any).showPicker();
    } else {
  ...
  const validateData = () => {
    ...
    checkRequired(formData.suspensionStart, "Suspension start");
    checkRequired(formData.suspensionEnd, "Suspension end");
    checkRequired(formData.hearingDate, "Date of hearing");
    checkRequired(formData.hearingTime, "Hearing time");
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(formData.suspensionStart) && /^\d{4}-\d{2}-\d{2}$/.test(formData.suspensionEnd)) {
      const suspensionStart = new Date(`${formData.suspensionStart}T00:00:00`);
      const suspensionEnd = new Date(`${formData.suspensionEnd}T00:00:00`);
      if (!Number.isNaN(suspensionStart.getTime()) && !Number.isNaN(suspensionEnd.getTime())) {
        if (suspensionEnd < suspensionStart) {
          throw new Error("Suspension end must be on or after Suspension start.");
        }
      }
    }
  ...
  const generatePDF = (data: PrecautionarySuspensionData, download = false) => {
    const doc = new jsPDF();
    ...
    const drawSuspensionDetailsSection = () => {
      const leftLabel = "Start:";
      const leftValue = valueOrLine(formatDate(data.suspensionStart));
      const rightLabel = "End:";
      const rightValue = valueOrLine(formatDate(data.suspensionEnd));
      const columnGap = 4;
      const columnWidth = (contentWidth - sectionPaddingX * 2 - columnGap) / 2;
      const measureValue = (value: string, labelWidth: number) => {
        const valueLines = doc.splitTextToSize(value || "________________________", columnWidth - labelWidth);
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.text("B. SUSPENSION DETAILS", margin + sectionPaddingX, startY + 4.7);
    ...
    const drawDisciplinaryHearingDetailsSection = () => {
      const dateLabel = "Date:";
      const timeLabel = "Time:";
      const chargesLabel = "Charge(s):";
      const dateValue = valueOrLine(formatDate(data.hearingDate));
      const timeValue = valueOrLine(formatTime(data.hearingTime));
      const chargesValue = valueOrLine(data.misconductTypes.join(", "));
      const columnGap = 4;
      const columnWidth = (contentWidth - sectionPaddingX * 2 - columnGap) / 2;
      const measureValue = (value: string, labelWidth: number, width: number) => {
        const valueLines = doc.splitTextToSize(value || "________________________", width - labelWidth);
        return { valueLines };
      };

      const date = measureValue(dateValue, leftColumnLabelWidth, columnWidth);
      const time = measureValue(timeValue, leftColumnLabelWidth, columnWidth);
      const charges = measureValue(chargesValue, leftColumnLabelWidth, contentWidth - sectionPaddingX * 2);
      const firstRowHeight = Math.max(
        Math.max(1, date.valueLines.length) * sectionLineGap + 1,
        Math.max(1, time.valueLines.length) * sectionLineGap + 1,
      );
      const chargesRowHeight = Math.max(1, charges.valueLines.length) * sectionLineGap + 1;
      const rowsHeight = firstRowHeight + chargesRowHeight;
      const sectionHeight = sectionHeaderHeight + sectionPaddingY + rowsHeight + sectionPaddingY;
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.text("C. DISCIPLINARY HEARING", margin + sectionPaddingX, startY + 4.7);
      ...
      doc.setFont("helvetica", "bold");
      doc.text(`${dateLabel} `, leftX, rowY);
      doc.setFont("helvetica", "normal");
      date.valueLines.forEach((line: string, idx: number) => {
        doc.text(line, leftX + leftColumnLabelWidth, rowY + idx * sectionLineGap);
      });

      doc.setFont("helvetica", "bold");
      doc.text(`${timeLabel} `, rightX, rowY);
      doc.setFont("helvetica", "normal");
      time.valueLines.forEach((line: string, idx: number) => {
        doc.text(line, rightX + leftColumnLabelWidth, rowY + idx * sectionLineGap);
      });

      const chargesY = rowY + firstRowHeight;
      doc.setFont("helvetica", "bold");
      doc.text(`${chargesLabel} `, leftX, chargesY);
      doc.setFont("helvetica", "normal");
      charges.valueLines.forEach((line: string, idx: number) => {
        doc.text(line, leftX + leftColumnLabelWidth, chargesY + idx * sectionLineGap);
      });
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text("PRECAUTIONARY SUSPENSION NOTICE", pageWidth / 2, y, { align: "center" });
    y += 7;

    drawEmployeeDetailsSection();

    drawSuspensionDetailsSection();

    drawDisciplinaryHearingDetailsSection();
    ...
    if (download) {
      ...
      toast({
        ...
        description: "Precautionary suspension notice has been generated.",
      });
      return;
    }
          ...
          >
            ...
            >
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        readOnly
                        className={getPrecautionarySuspensionModalInputClass(Boolean(profile?.company_name))}
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        readOnly
                        className={getPrecautionarySuspensionModalInputClass(Boolean(profile?.registration_number))}
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        readOnly
                        className={getPrecautionarySuspensionModalInputClass(Boolean(profile?.physical_address))}
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        className={getPrecautionarySuspensionModalInputClass(formData.tradingName.trim().length > 0)}
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        className={getPrecautionarySuspensionModalInputClass(formData.employerContact.trim().length > 0)}
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        className={getPrecautionarySuspensionModalInputClass(formData.employerEmail.trim().length > 0)}
                      />
                    </div>
              ...
              {activeStep === 1 && (
                <div className="space-y-3">
                    <div className="space-y-2.5">
                      ...
                      >
                        <SelectTrigger className={`${getPrecautionarySuspensionModalSelectTriggerClass(selectedEmployeeId.trim().length > 0)} ${precautionarySuspensionModalDropdownToneClass}`}>
                          <SelectValue placeholder="Select from saved employees or fill manually" />
                        </SelectTrigger>
                        <SelectContent
                          hideScrollButtons
                          className={`${precautionarySuspensionModalSelectContentClass} w-[var(--radix-select-trigger-width)] p-0`}
                        >
                          ...
                          {searchedEmployees.length > 0 ? (
                            searchedEmployees.map((employee) => (
                              <SelectItem key={employee.id} value={employee.id} className={precautionarySuspensionModalSelectItemClass}>
                    ...
                    <div className="grid md:grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          className={getPrecautionarySuspensionModalInputClass(formData.employeeName.trim().length > 0)}
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          className={getPrecautionarySuspensionModalInputClass(formData.employeeSurname.trim().length > 0)}
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        >
                          <SelectTrigger className={`${getPrecautionarySuspensionModalSelectTriggerClass(Boolean(formData.idType))} ${precautionarySuspensionModalDropdownToneClass}`}>
                            <SelectValue placeholder="Choose document type" />
                          </SelectTrigger>
                          <SelectContent className={precautionarySuspensionModalSelectContentClass}>
                            <SelectItem value="id" className={precautionarySuspensionModalSelectItemClass}>ID Number</SelectItem>
                            <SelectItem value="passport" className={precautionarySuspensionModalSelectItemClass}>Passport Number</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          className={`${getPrecautionarySuspensionModalInputClass(
                            formData.idType === "id"
              ...
              {activeStep === 2 && (
                <div className="space-y-3">
                  <div className="grid md:grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <Label htmlFor="suspensionStart" className={`${modalFieldLabelClass} inline-flex items-center gap-1`}>
                        Suspension Start <span className="text-red-500">*</span>
                      </Label>
                      <div className="flex items-start gap-2">
                        <Input
                          ...
                          value={formData.suspensionStart ? toDisplayDate(formData.suspensionStart) : ""}
                          onClick={openSuspensionStartDatePicker}
                          onFocus={openSuspensionStartDatePicker}
                          onKeyDown={(e) => {
                            if (e.key === "Enter" || e.key === " ") {
                              e.preventDefault();
                              openSuspensionStartDatePicker();
                            }
                          }}
                          className={`${getPrecautionarySuspensionModalInputClass(formData.suspensionStart.trim().length > 0)} flex-1 cursor-pointer placeholder:!text-[11px] placeholder:!font-normal placeholder:!text-slate-400`}
                        />
                        <input
                          ref={suspensionStartDatePickerRef}
                          type="date"
                          value={formData.suspensionStart && /^\d{4}-\d{2}-\d{2}$/.test(formData.suspensionStart) ? formData.suspensionStart : ""}
                          onChange={(e) => setFormData((prev) => ({ ...prev, suspensionStart: e.target.value }))}
                          className="sr-only"
                          aria-hidden="true"
                          tabIndex={-1}
                        />
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="suspensionEnd" className={`${modalFieldLabelClass} inline-flex items-center gap-1`}>
                        Suspension End <span className="text-red-500">*</span>
                      </Label>
                      <div className="flex items-start gap-2">
                        <Input
                          id="suspensionEnd"
                          ...
                          readOnly
                          placeholder="Please select a date"
                          value={formData.suspensionEnd ? toDisplayDate(formData.suspensionEnd) : ""}
                          onClick={openSuspensionEndDatePicker}
                          onFocus={openSuspensionEndDatePicker}
                          onKeyDown={(e) => {
                            if (e.key === "Enter" || e.key === " ") {
                              e.preventDefault();
                              openSuspensionEndDatePicker();
                            }
                          }}
                          className={`${getPrecautionarySuspensionModalInputClass(formData.suspensionEnd.trim().length > 0)} flex-1 cursor-pointer placeholder:!text-[11px] placeholder:!font-normal placeholder:!text-slate-400`}
                        />
                        <input
                          ref={suspensionEndDatePickerRef}
                          type="date"
                          value={formData.suspensionEnd && /^\d{4}-\d{2}-\d{2}$/.test(formData.suspensionEnd) ? formData.suspensionEnd : ""}
                          onChange={(e) => setFormData((prev) => ({ ...prev, suspensionEnd: e.target.value }))}
                          className="sr-only"
                          aria-hidden="true"
                          tabIndex={-1}
                        />
                    ...
                    <div className="space-y-1.5">
                        ...
                        <TooltipProvider delayDuration={0}>
                          <Tooltip>
                            ...
                            <TooltipContent side="top" className={fixedTooltipContentClass}>
                              Ensure the hearing date reflects when the disciplinary process meeting will be held during the precautionary suspension process.
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </Label>
                      <div className="flex items-start gap-2">
                        <Input
                          ...
                          className={`${getPrecautionarySuspensionModalInputClass(formData.hearingDate.trim().length > 0)} flex-1 cursor-pointer placeholder:!text-[11px] placeholder:!font-normal placeholder:!text-slate-400`}
                        />
                        <input
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div key={hearingTimeFieldVersion} className="relative">
                        ...
                        >
                          <SelectTrigger
                            id="hearingTime"
                            className={`${getPrecautionarySuspensionModalSelectTriggerClass(Boolean(formData.hearingTime.trim()) && !hearingTimeFocused)} ${precautionarySuspensionModalDropdownToneClass}`}
                          >
                          ...
                          <SelectContent hideScrollButtons className={precautionarySuspensionModalSelectContentClass}>
                            {hearingTimeOptions.map((option) => (
                              <SelectItem key={option.value} value={option.value} className={precautionarySuspensionModalSelectItemClass}>
                                {option.label}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        ...
                        {hearingTimeFocused ? (
                          <div className="absolute inset-0 z-20">
                            <Input
                              ...
                              className={`h-[34px] pr-11 ${getPrecautionarySuspensionModalInputClass(Boolean(formData.hearingTime.trim()))}`}
                            />
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent side="top" className={fixedTooltipContentClass}>
                              ...
                              {formData.employeeName || formData.employeeSurname
                                ...
                                : "the employee"}{" "}
                              was found guilty following the disciplinary process.
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </Label>
            ...
            {validatedPreview ? (
              <div className="space-y-8">
                <FirstPagePreview data={validatedPreview} profile={profile} logoPreviewUrl={companyLogoPreview || validatedPreview.companyLogoDataUrl}>
                  <div className="w-full space-y-4 pt-2 text-[11px] leading-relaxed text-black">
                    <h2 className="text-center text-[14px] font-bold uppercase">Precautionary Suspension Notice</h2>
                    ...
                    <section className="overflow-hidden rounded border border-slate-300">
                      <div className="w-full bg-slate-100 px-3 py-1.5 text-[11px] font-semibold uppercase">
                        B. Suspension Details
                      </div>
                      <div className="grid grid-cols-1 gap-2 px-3 py-2 md:grid-cols-2">
                        <div className="grid grid-cols-[124px,1fr]">
                          <span className="font-semibold">Start:</span>
                          <span>{formatDate(validatedPreview.suspensionStart) || "________________________"}</span>
                        </div>
                        <div className="grid grid-cols-[124px,1fr]">
                          <span className="font-semibold">End:</span>
                          <span>{formatDate(validatedPreview.suspensionEnd) || "________________________"}</span>
                        </div>
                      </div>
                    </section>
                    ...
                    <section className="overflow-hidden rounded border border-slate-300">
                      ...
                      <div className="px-3 py-2">
                        <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                          ...
                          <div className="grid grid-cols-[124px,1fr]">
                            <span className="font-semibold">Hearing Time:</span>
                            <span>{formatTime(validatedPreview.hearingTime) || "________________________"}</span>
                          </div>
                          <div className="grid grid-cols-[124px,1fr] md:col-span-2">
                            <span className="font-semibold">Charge(s):</span>
                            <span>{validatedPreview.misconductTypes.join(", ") || "________________________"}</span>
                          </div>
                        </div>
                      </div>
                    </section>
              ...
              <div className="flex h-full items-center justify-center">
                <p className="text-sm text-muted-foreground">Complete the form to preview the precautionary suspension notice.</p>
              </div>
            )}
          </ScrollArea>
                {!useExternalShell ? (
              ...
              <div className="space-y-1 p-3">
                {filteredMisconductTypes.length === 0 ? (
                  ...
                  filteredMisconductTypes.map((type) => (
                    <label
                      key={type}
                      className={`flex items-center gap-2 cursor-pointer rounded px-2 py-1 hover:bg-blue-50/70 hover:text-blue-600 focus-within:bg-blue-50/70 ${precautionarySuspensionModalSelectItemClass}`}
                    >
                      <Checkbox
